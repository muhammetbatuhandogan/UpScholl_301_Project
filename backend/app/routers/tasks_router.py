...
async def get_tasks(
    ...
    return TaskListResponse(items=list_tasks(db, current_user.id))
...
async def put_task(
    ...
    updated_task = update_task(db, current_user.id, task_id, payload)
...
async def remove_task(
    ...
    is_deleted = delete_task(db, current_user.id, task_id)
    if not is_deleted:
        raise HTTPException(status_code=404, detail="Task not found.")
